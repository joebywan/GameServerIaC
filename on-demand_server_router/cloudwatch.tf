...
resource "aws_cloudwatch_log_subscription_filter" "log_subscription_filter" {
    name            = "${local.workload_name}-subscription-filter"
...
resource "aws_lambda_permission" "allow_cloudwatch_to_call_lambda" {
    ...
    source_arn    = "arn:aws:logs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:log-group:${local.source_log_group_name}:*"
}